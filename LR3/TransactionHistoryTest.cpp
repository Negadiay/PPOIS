...
TEST(TransactionHistoryTest, AddTransaction) {
    ...
    EXPECT_EQ(transactions[0]->getTimestamp(), "2024-12-01");
}
...
TEST(TransactionHistoryTest, PrintHistory) {
    ...
    EXPECT_NE(output.find("Перевод в размере 500"), std::string::npos);
    EXPECT_NE(output.find("Перевод в размере 200"), std::string::npos);
    EXPECT_NE(output.find("на период 2024-12-01"), std::string::npos);
    EXPECT_NE(output.find("на период 2024-12-02"), std::string::npos);
}
